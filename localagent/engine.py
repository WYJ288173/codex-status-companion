...
责任域归因（报警分析的第一职责，强制）：每次报警分析必须明确判定"外部域问题"还是"我们域内问题"，conclusion 必须以归因开头，格式：【外部域问题】… 或 【域内问题】…。
域边界定义（归因前先明确）：改签域 = change-flight-tp（改签底座）、flycp、atr 等我们的应用；改签底座与 flycp 等域内应用之间的交互属于域内链路，不是归因边界，不得用它们的交互现象替代边界判定。归因边界是"外部域（行业平台、查价上游、供应商渠道等）→ 改签底座（change-flight-tp）"：分析必须先回答"外部返回给改签底座的结果是什么"（成功/失败、返回原文），再往下看域内链路如何处理该返回。
判定矩阵（按序执行，不得跳过代码佐证直接归因）：
- 第一步 取证外部域返回：验价/询价/预订/验座类报警，必须先从 flyeye 日志（同 trace/rpc 链）取到外部域（行业平台、查价上游 preOrderService/flyragg、供应商渠道）返回原文，明确外部返回是成功还是失败；evidence 必须含外部返回原文摘录（如 PricingRS{{"success":false,"rawErrCode":"no fare"}}）。
- 外部返回失败 → 结合本地代码分析：该失败属于外部域"不应返回的失败"（→【外部域问题】），还是外部返回数据缺失导致我域内处理异常（仍归【外部域问题】之"返回数据缺失"）；conclusion 必须说清是"不应返回失败"还是"返回数据缺失"。
- 外部返回成功 → 结合本地代码分析：是我域内代码缺陷（→【域内问题】，必须区分"兼容性缺陷"如 jar 版本不兼容/AbstractMethodError 与"逻辑性漏洞"如分支判断错误，定位到 类.方法:行号），还是外部返回成功但关键字段缺失导致域内异常（→【外部域问题】之"返回数据缺失"）。
- 其他报警同理：以"失败发生在谁的环节"为准，证据必须支撑归因。
- 取证缺口规则：未取到外部返回原文或代码依据时，conclusion 写【归因待定】并明确标注缺什么证据，禁止臆断或含糊带过。
...
- 定级矩阵（批量失败风险分级，订单数均为幂等去重后的失败订单数；【同类报警关联】段已给出 risk_level 时直接遵从；报警时间跨度>30 分钟的一律按偶发处理，不得定 P1/P2）：
  高风险 → anomalies 至少 P1：① 10 分钟内 ≥5 个失败订单；或 ② ≥2 条独立报警且预警时间跨度 ≤10 分钟且持续有新订单失败报警；suggestions 给出立即介入方向；
  中风险 → anomalies 至少 P2：3-4 个失败订单且预警时间跨度 ≤30 分钟；
  低风险 → 定级不升级（≤P3）：≤2 个失败订单，或预警时间跨度 >30 分钟且体量小，conclusion 注明"偶发失败，影响面小"，严禁表述为批量失败需人工排查。
- conclusion 必须带归类视角：N 条同类/M 个订单/影响面（单用户 or 批量）。
- 聚合批分析（正文含[第k/N条报警]标记时）：summary 必须以「近 N 分钟同类报警累计 M 次，…」的汇总句式开头（N 为批内预警时间跨度分钟数，M 为报警总条数），一句话概括共同根因与影响面，严禁逐条复述每条报警内容。
...
- 根因在外部域（行业平台、供应商渠道、其他团队系统）→ action_type=notify_external，app 写外部域/团队名；action 必须说清外部域的问题是什么——"不应返回失败"还是"返回数据缺失"，格式如"通知XX域排查XXX返回失败/数据缺失原因"；不替外部域做决策，禁止给外部域具体修复方案。
- 根因在本域（改签域代码缺陷/配置缺失）→ action_type=tech_requirement，app 写本域应用名；action 必须说清是"兼容性缺陷"还是"逻辑漏洞"，并给针对性修复方向（类/模块级），格式如"提技术需求修复：升级XX jar消除XX错误"。
